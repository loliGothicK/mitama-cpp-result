...
template <class T>
struct has_type<T, std::void_t<typename std::decay_t<T>::type>>
    : std::true_type {};
...
template <class T>
struct has_value<T, std::void_t<decltype(std::decay_t<T>::value)>>
    : std::true_type {};

template <class T>
struct force_add_const : std::add_const<T> {};
template <class T>
struct force_add_const<T&> {
...
template <class T>
using force_add_const_t = typename force_add_const<T>::type;
...
template <class T, class U>
struct is_less_comparable_with<
    T, U,
    std::void_t<decltype(std::declval<const T&>() < std::declval<const U&>())>>
    : std::true_type {};
...
namespace detail {
  template <class, class, class = void>
  struct is_tuple_like_detail : std::false_type {};

  template <class T, std::size_t... I>
  struct is_tuple_like_detail<
  ...
  struct is_tuple_like_impl
      : is_tuple_like_detail<
            TupleLike, std::make_index_sequence<std::tuple_size_v<TupleLike>>> {
  };
} // namespace detail
...
struct is_tuple_like
    : std::conjunction<
          has_value<std::tuple_size<T>>, detail::is_tuple_like_impl<T>> {};
...
struct is_range<
    Range,
    std::void_t<
        decltype(*std::begin(std::declval<std::decay_t<Range>>()), std::begin(std::declval<std::decay_t<Range>>()) != std::end(std::declval<std::decay_t<Range>>()))>>
    : std::true_type {};
